...
public class PropToMultiInst
  ...
  public boolean batchFinished() {
    ...
    //Instance inst = new SparseInstance(bagInsts.numAttributes());
    ...
    for(int i = 0; i < input.numInstances(); i++) {
      ...
      SparseInstance in = (SparseInstance)input.instance(i);
      double[] tempValues = new double[input.numAttributes() - 2];
      int nv = in.numValues();
      for (int j = 0; j < nv; j++) {
        int attrIndex = in.index(j);
        if (attrIndex > 0                             // it is not the bag's ID
            && attrIndex < input.numAttributes() - 1) // it is not the class value
          //inst.setValue(attrIndex - 1, in.valueSparse(j));
        tempValues[attrIndex - 1] = in.valueSparse(j);
      }
      Instance inst = new SparseInstance(input.instance(i).weight(), tempValues);
      inst.setDataset(bagInsts);
      /*
  ...
  public static void main(String[] args) {
    runFilter(new PropToMultiInst(), args);
  }
}
