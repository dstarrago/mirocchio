...
public class RemoveRare
  ...
  /** Frequencia más baja admisible. Los atributos que aparezcan en un
   * número de instancias menor que el especificado por lowerFrequency
   * serán eliminados.
   */
  protected int lowerFrequency = 5;

  /** Frequencia más baja admisible expresada en porciento. Se utiliza para
   * calcular el valor de lowerFrequency. Si es igual a cero, se ignora.
   */
  protected int lowerFrequencyPercent = 1;
  ...
  /**
   * Número de instancias cuyo valor de atributo es mayor que cero
   * @param data conjunto de datos
   * @param attrIndex índice del atributo de interés
   * @return retorna el número de instancias para las cuales el valor del
   * atributo indicado es mayor que cero.
   */
  private int greaterThanZero(Instances data, int attrIndex) {
    int count = 0;
    for (int i = 0; i < data.numInstances(); i++) {
      if (data.instance(i).value(attrIndex) > 0)
        count++;
    }
    return count;
  }
  ...
  public boolean batchFinished() throws Exception {
    ...
    if (m_removeFilter == null) {
      ...
      int numToDelete = 0;
      if (lowerFrequencyPercent > 0)
        lowerFrequency = lowerFrequencyPercent * toFilter.numInstances()/100;
      for(int i = 0; i < toFilter.numAttributes(); i++) {
        if (i==toFilter.classIndex()) continue; // skip class
        int greaterThanZero = greaterThanZero(toFilter, i);
        if (greaterThanZero < lowerFrequency)
  ...
  public void setLowerFrequency(int value) {
    if (value > 1)
      lowerFrequency = value;
  }

  public void setLowerFrequencyPercent(int percent) {
    lowerFrequencyPercent = percent;
  }
